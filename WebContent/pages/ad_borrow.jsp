	...
	<center>
		<div class="table_content" style="margin: 0">
			<table>
				...
				<%
					...
					if(search_type == null || search_type.equals("null") || keyword == ""){
						...
						list = brd.search(search_type, keyword);
					}
					...
					if(list.isEmpty()){
						...
						for(int i = (index - 1) * contentNum;i < index * contentNum && i < list.size() ;i++){
							...
							out.print("<tr>" 
									...
									+ "<td>" + "<a href='javascript:void(0);' onclick='deleteBorrow(\"" + list.get(i).getReaderId() + "\",\"" + list.get(i).getISBN() + "\")'>删除</a>" + "</td>" 
									+ "</tr>");
						}
					}
				%>
			...
			<script type="text/javascript">
				function deleteBorrow(readerId,isbn) {
					layer.confirm('确定要删除吗?', {
						icon : 3,
						shade : 0,
						title : '确认'
					}, function(index) {
						window.location.href = '/shujuku/DeleteBorrow?readerId=' + readerId + '&isbn=' + isbn;
						layer.close(index);
					});
				}
			</script>
		...
		function pageFun(pageCtr){
			if(pageCtr == 'last'){
				if(<%=Integer.parseInt((String)session.getAttribute("pageIndex")) %> == 1){
					...
					document.mypage.action="ad_borrow.jsp?search_type=" + search_type + "&keyword=" + keyword + "&pageCtr=" + pageCtr;
				...
				if(parseInt(document.getElementById('jumpto').value) > <%=list.size() / contentNum + 1 %> || parseInt(document.getElementById('jumpto').value) < 1){
					...
					document.mypage.action="ad_borrow.jsp?search_type=" + search_type + "&keyword=" + keyword + "&pageCtr=" + pageCtr + "&jumpto=" + document.getElementById('jumpto').value;
					document.mypage.submit();
 				}
			}else{
				document.mypage.action="ad_borrow.jsp?search_type=" + search_type + "&keyword=" + keyword + "&pageCtr=" + pageCtr;
